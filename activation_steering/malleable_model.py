...
from .leash_layer import LeashLayer
from .utils import custom_progress
...
class MalleableModel(torch.nn.Module):
    ...
    def multibehavior(
        self,
        behavior_vectors: List["SteeringVector"],
        ...
        **kwargs,
        ...
        """
        Apply multiple behavior steering vectors without any conditioning.

        Args:
            behavior_vectors: List of steering vectors to apply.
            behavior_layer_ids: Layers corresponding to each behavior vector.
            behavior_vector_strengths: Scaling factor for each behavior vector.
            use_explained_variance: Whether to scale vectors by explained variance.
            use_ooi_preventive_normalization: Enable preventive normalization.
            apply_behavior_on_first_call: Apply behavior on first forward call.
            **kwargs: Forwarded to LeashLayer.steer.
        """
        log("Multi-behavior steering...", style="bold", class_name="MalleableModel")

        assert len(behavior_vectors) == len(behavior_layer_ids) == len(behavior_vector_strengths), (
            "behavior_vectors, behavior_layer_ids, and behavior_vector_strengths must have equal lengths."
        )

        layers = get_model_layer_list(self.model)
        num_layers = len(layers)

        layer_to_vectors: dict[int, list[torch.Tensor]] = defaultdict(list)
        behavior_layers = {i: False for i in range(num_layers)}

        for vector, layer_ids, strength in zip(behavior_vectors, behavior_layer_ids, behavior_vector_strengths):
            for layer_id in layer_ids:
                behavior_layers[layer_id] = True
                direction = vector.directions[layer_id]
                if use_explained_variance:
                    direction = self.use_explained_variance(vector)
                layer_to_vectors[layer_id].append(
                    torch.tensor(strength * direction, dtype=self.model.dtype, device=self.model.device)
                )

        LeashLayer.condition_layers = {i: False for i in range(num_layers)}
        LeashLayer.behavior_layers = behavior_layers
        ...
        for layer_id in range(num_layers):
            ...
            if layer_id in layer_to_vectors:
                behavior_tensor = torch.stack(layer_to_vectors[layer_id]).sum(dim=0)
    ...
    def respond(
        ...
        if use_chat_template:
            if system is None:
                system = "You are a supportive, practical advisor. If the user asks you for advice between choices, give your opinion on what they should choose. First choose an option, then give an explanation. ALWAYS CHOOSE ONE OF THE OPTIONS PROVIDED. Keep your answers concise and to the point."
                # system = "You are a supportive, practical advisor. If the user asks you for advice between choices, give your opinion on what they should choose. First give an explanation, then choose an option. ALWAYS CHOOSE ONE OF THE OPTIONS PROVIDED. Keep your answers concise and to the point."
        ...
        if settings is None:
            print("Using default generation settings.")
            settings = {
