...
class LeashLayer(nn.Module):
    ...
    def __init__(self, layer: nn.Module, layer_id: int) -> None:
        ...
        # Qwen3 expects each decoder layer to expose `attention_type`.
        # Preserve that attribute on the wrapper when present.
    ...
    def _apply_single_behavior(self, hidden_states):
        ...
        """
        has_active_conditions = self._has_active_condition_layers()
        should_apply = not has_active_conditions or LeashLayer.condition_met[0]
        ...
        if should_apply:
            ...
            if LeashLayer.forward_calls[self.layer_id] == 1:
                if self.apply_behavior_on_first_call:
                    self._apply_behavior_control(hidden_states, control, self.params, has_active_conditions)
                else:
    ...
    def _apply_multi_behaviors(self, hidden_states):
        ...
        for rule in self.rules:
            ...
            if self._evaluate_rule(rule) and \
                ...
                if LeashLayer.forward_calls[self.layer_id] == 1:
                    if self.apply_behavior_on_first_call:
                        self._apply_behavior_control(
                            hidden_states,
                            control,
                            self.params[behavior_index],
    ...
    def _has_active_condition_layers(self):
        """
        Return whether steering is configured with any condition layers.

        Unconditional behavior steering is batch-safe: the same behavior vector
        should be applied to every example in the batch. Conditional steering in
        this wrapper still tracks condition state globally, so it keeps the
        historical first-example behavior.
        """
        condition_layers = LeashLayer.condition_layers
        if condition_layers is None:
            return False
        if isinstance(condition_layers, dict):
            return any(condition_layers.values())
        return any(any(layers.values()) for layers in condition_layers)

    def _apply_behavior_control(self, hidden_states, control, params, has_active_conditions):
        if has_active_conditions:
            hidden_states[0] = params.operator(hidden_states[0], control)
            log(
                "    Applying conditional behavior vector to first batch item",
                class_name="LeashLayer",
            )
        else:
            hidden_states[:] = params.operator(hidden_states, control)
            log(f"    Applying behavior vector to batch of {hidden_states.shape[0]}", class_name="LeashLayer")
