...
from .malleable_model import MalleableModel, get_model_layer_list
from .utils import ContrastivePair, custom_progress
from .steering_dataset import SteeringDataset
from .config import log, GlobalConfig
            ...
            for layer_id in hidden_layer_ids:
                ...
                for i, batch_hidden in enumerate(out.hidden_states[hidden_idx]):
                    if accumulate_last_x_tokens == "all":
                        ...
                        if suffixes:
                            example_idx = batch_start_idx + i
                            pair_idx, suffix_idx = divmod(example_idx, 2)
                            suffix_text = suffixes[pair_idx][suffix_idx]
                            suffix_tokens = tokenizer.encode(suffix_text, add_special_tokens=False)
                            actual_length = int(sequence_lengths[i])

                            if suffix_tokens:
                                suffix_start = max(0, actual_length - len(suffix_tokens))
                                suffix_hidden = batch_hidden[suffix_start:actual_length, :]
                                accumulated_hidden_state = torch.mean(suffix_hidden, dim=0)
                            else:
                                accumulated_hidden_state = batch_hidden[actual_length - 1, :]
                        else:
